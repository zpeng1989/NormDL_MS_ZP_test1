...
from visual import VisObj, pca_for_dict
...
class BatchEffectTrainer:
    def __init__(
        ...
        self.device = device
        self.encoder_hiddens = [1000, 1000]
        self.decoder_hiddens = [1000, 1000]
        self.disc_b_hiddens = [250, 250]
        self.disc_o_hiddens = [250, 250]
        self.bottle_num = 500
        ...
        # loss
        self.use_batch_for_order = True
        self.lambda_b, self.lambda_o = 1.0, 1.0
        ...
        # optimizer
        self.lr_rec = 0.0002
        self.lr_disc_b = 0.005
        self.lr_disc_o = 0.0005
        ...
        # training
        self.epoch = 1710
        self.rec_epoch, self.disc_epoch, self.iter_epoch = (1000, 10, 700)
        self.bs, self.nw = 64, 12
        self.train_with_qc = "all"
    ...
    def fit(self, datas):
        ...
        for e in range(self.epoch):
            ...
            all_reses_dict, qc_loss = self.generate(all_data, verbose=False, compute_qc_loss=True)
    ...
    def generate(self, data_loader, verbose=True, compute_qc_loss=False):
        ...
        if isinstance(data_loader, data.Dataset):
            data_loader = data.DataLoader(data_loader, batch_size=self.bs, num_workers=self.nw)
        ...
        with torch.no_grad():
            ...
            for batch_x, batch_y in iterator:
                ...
                batch_ys = [torch.eye(self.batch_label_num)[batch_y[:, 1].long()].to(hidden), batch_y[:, [0]]]
    ...
    def _build_model(self):
        ...
        self.optimizers = {
            ...
            "disc_b": optimizer_obj(self.models['disc_b'].parameters(), lr=self.lr_disc_b),
            "disc_o": optimizer_obj(self.models['disc_o'].parameters(), lr=self.lr_disc_o)
        }
    ...
    def _forward_autoencode(self, batch_x, batch_y):
        ...
        with torch.enable_grad():
            ...
            # decoder
            batch_ys = [
                torch.eye(self.batch_label_num)[batch_y[:, 1].long()].to(hidden), batch_y[:, [0]]
            ]
            #print('---------test-------batch_ys------------')
            #print(batch_ys)
            #print('---------end--------batch_ys------------')
            ## 数据分为两部分，第一部分是分组，第二部分是排序
            batch_ys = torch.cat(batch_ys, dim=1)
            #print(batch_ys)
            hidden_be = hidden + self.models['map'](batch_ys)
